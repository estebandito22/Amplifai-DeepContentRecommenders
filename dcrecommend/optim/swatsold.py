...
class Swats(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad, sgd=None,
                        sgd_lr=None, lk=0, phase='ADAM', epsilon=epsilon)
    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            ...
            group.setdefault('sgd', None)
            group.setdefault('sgd_lr', None)
            group.setdefault('lk', 0)
            group.setdefault('phase', 'ADAM')
            group.setdefault('epsilon', self.epsilon)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if pk.dot(grad.view(1, -1).squeeze()) != 0:
                    ...
                    if state['step'] > 2:
                        print("Switching from ADAM to SGD at step {}".format(
                            state['step']))
                        group['phase'] = 'SGD'
                        group['sgd_lr'] = torch.div(
                            group['lk'], 1 - beta2 ** state['step'])
                        group['sgd'] = SGD(
                            self.param_groups, group['sgd_lr'], beta1)
