...
class TrueDcueNetMel1DMultiBn(nn.Module):
    ...
    def __init__(self, dict_args):
        ...
        Args
            dict_args: dictionary containing the following keys:
                hidden_size: the output size of the network.  Corresponds to
                    ...
                    users and songs.
        """
        super(TrueDcueNetMel1DMultiBn, self).__init__()
        ...
        # batch size x self.output_size x 1
        # batch size x 128 x 1

        self.fc = nn.Linear(self.output_size, self.output_size)
        # batch size x 1 x 128

        self.outsize = [self.hidden_size, [33, 8, 2, 1, 1, 1]]
    ...
    def forward(self, x):
        ...
        return [tp1, tp2, tp3, tp4, tp5, x]  # (list) batch_size x context_dim (self.hidden_size) x context_size (time dim)
